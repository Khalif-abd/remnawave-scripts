...
| `renew-ssl` | Обновить/перевыпустить SSL сертификат (Nginx и Caddy) |
| `reissue-cert` | 🔐 Принудительно перевыпустить сертификат Caddy одной командой |
...
| Перевыпуск SSL | `selfsteal reissue-cert` (форс) | `selfsteal renew-ssl` → Force |
...
#### Проблема: Caddy «застрял» на старом/самоподписанном сертификате
Если Caddy выдал внутренний (self-signed) сертификат или сертификат завис, можно
**принудительно перевыпустить** его одной командой — без переустановки:
```bash
selfsteal reissue-cert        # интерактивно (с подтверждением)
selfsteal reissue-cert -f     # без подтверждения (для скриптов/cron)
```
Команда останавливает Caddy, **делает резервную копию** текущего сертификата,
очищает хранилище (`/data/caddy/certificates`) и запускает Caddy заново — он
запрашивает свежий сертификат через ACME (HTTP-01 на порту 80). Скрипт ждёт до 90 с
и проверяет, что выдан **публично доверенный** сертификат (по серийному номеру и
издателю). Если выпуск не удался (закрыт порт 80, неверный DNS, лимиты CA), скрипт
**автоматически восстанавливает прежний сертификат**, чтобы маскировка не осталась
без рабочего TLS.

> ⚠️ Let's Encrypt ограничивает выпуск дубликатов (~5 в неделю на домен). Используйте
> `reissue-cert` для восстановления, а не по расписанию.

Аналогично работает `selfsteal renew-ssl` (для Caddy выполняет тот же форс-перевыпуск,
для Nginx обновляет сертификат через acme.sh).
